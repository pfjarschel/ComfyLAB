...
def find_blueprint_dependencies(blueprint: dict) -> Tuple[List[Path], List[Path]]:
    ...
    # We want to scan recursively if clusters contain other custom blocks/clusters
    blueprints_to_scan = [blueprint]
    scanned_clusters = set()
    
    while blueprints_to_scan:
        current_bp = blueprints_to_scan.pop(0)
        for block in current_bp.get("blocks", []):
            block_type = block.get("type")
            if block_type == "actionBlock" and isinstance(block.get("data"), dict):
                block_type = block.get("data", {}).get("action")
                
            if not block_type or block_type not in BLOCK_REGISTRY:
                continue
            ...
            if hasattr(cls, "_cluster_file_path") and cls._cluster_file_path:
                ...
                if cluster_path.exists() and cluster_path not in cluster_files:
                    ...
                    if block_type not in scanned_clusters:
                        scanned_clusters.add(block_type)
                        try:
                            ...
                            logger.error(f"Failed to parse cluster internal blueprint for {block_type}: {e}")
            else:
    ...
    return list(custom_block_files), list(cluster_files)
...
async def export_package(payload: PackageExportPayload):
    """Bundles a blueprint and its custom block/cluster dependencies into a .cfy package."""
    ws_path = get_workspace_path()
    packages_dir = ws_path / "packages"
    packages_dir.mkdir(parents=True, exist_ok=True)
...
async def import_package(payload: PackageImportPayload):
    ...
    if not payload.import_permanent:
        ...
        target_blocks_dir = ws_path / "blocks" / ".temp"
        target_clusters_dir = ws_path / "clusters" / ".temp"
        
        # Clean up any previous temp extraction to avoid leftovers
        for d in [target_bp_dir, target_blocks_dir, target_clusters_dir]:
            if d.exists():
                shutil.rmtree(d)
    else:
    ...
    target_blocks_dir.mkdir(parents=True, exist_ok=True)
...
async def clear_temporary_package_files():
    """Purges all .temp subdirectories under workspace blueprints, blocks, and clusters and triggers a registry reload."""
    ws_path = get_workspace_path()
    try:
