...
class Pin:
    """Base class for all input/output pins on a node."""
...
class ExecutionContext:
    ...
    async def pull(self, node_id: str, input_pin_name: str) -> Any:
        """
        Pulls data from the connection connected to the input pin on node_id.
        If no connection exists, returns the default value of the pin.
        """
        return await self.engine.pull_data(node_id, input_pin_name, self)
    ...
    def get_cached(self, node_id: str, pin_name: str) -> tuple[bool, Any]:
    ...
    async def send_telemetry(self, node_id: str, data: Any):
        ...
        if hasattr(self.engine, "send_telemetry"):
            await self.engine.send_telemetry(self.run_id, node_id, data)


class BaseNode:
    """
    Abstract base class for all ComfyLAB nodes.
    ...
    async def execute(self, context: ExecutionContext, trigger_pin: str) -> Optional[str]:
        """
        Executed when an execution token reaches this node.
    ...
    async def clear_data(self) -> None:
        """
        Resets any transient execution or output data stored on the node.
    ...
    async def _device_initialize(self, context: "ExecutionContext", device: Any) -> None:
        """
        Optional hook for device-connecting nodes. Called after a VISA/connection
    ...
    async def _device_teardown(self, device: Any, lock_manager: Any) -> None:
        """
        Optional hook for device-connecting nodes. Called during teardown,
