...
from comfylab.nodes.base import ExecutionContext, ExecIn, ExecOut, DataIn, DataOut
from comfylab.nodes.base_script import BaseScriptNode, parse_decorators
...
@register_node("script/python")
class PythonScriptNode(BaseScriptNode):
    icon = "🐍"
    ...
    async def execute(self, context: ExecutionContext, trigger_pin: str) -> Optional[str]:
        ...
        if not code.strip():
            raise ValueError("Script node has no code to execute.")
        ...
        try:
            ...
            raise TimeoutError(f"Script node '{self.id}' exceeded timeout of {timeout}s.")
        finally:
