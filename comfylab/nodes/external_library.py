...
"""
ComfyLAB DLL/SO Library Bridge Nodes
...
logger = logging.getLogger("comfylab.nodes.external_library")
...
# ---------------------------------------------------------------------------
# Node 1 — Library Loader
# ---------------------------------------------------------------------------

@register_node(r"dll\/SO/load")
class LibraryLoadNode(BaseNode):
    """
    Loads a native shared library (.dll / .so) and outputs a handle for use
    with Library Call nodes. Calling convention (cdecl vs stdcall) is detected
    ...
    released automatically on teardown, consistent with VISA device nodes.
    """
    icon = "🔗"
    display_name = "Load DLL/SO"
    description = (
        "Loads a native shared library (.dll on Windows, .so on Linux) and "
        "outputs the handle. Connect to a 'Call DLL/SO' node to invoke functions."
    )
...
@register_node(r"dll\/SO/call")
class LibraryCallNode(BaseNode):
    """
    ...
    argument entry has:
        name      — matches the DataIn/DataOut pin name on this node
    ...
    description = (
        ...
        "argument signature. Click '⚙️ Edit Signature' on the node to define "
        "the function's arguments and their C types."
    )

    # Static pins always present on every Library Call node instance.
    inputs_def = [
        ExecIn("Call"),
    ...
    async def execute(self, context: ExecutionContext, trigger_pin: str) -> Optional[str]:
        ...
        if lib is None:
            raise ValueError(
                "Call DLL/SO: No library handle. Connect a 'Load DLL/SO' node to the 'Library' input."
            )
