...
@register_node("visa/signal_generator/connect")
class PFJSigGenConnectNode(BaseDeviceConnectNode):
...
@register_node("visa/signal_generator/config_wave")
class PFJSigGenConfigWaveNode(BaseNode):
    ...
    async def execute(self, context: ExecutionContext, trigger_pin: str) -> Optional[str]:
        ...
        if not device:
            raise ValueError("No device connection handle supplied to PFJSigGen Config Wave node.")
...
@register_node("visa/signal_generator/config_chirp")
class PFJSigGenConfigChirpNode(BaseNode):
    ...
    async def execute(self, context: ExecutionContext, trigger_pin: str) -> Optional[str]:
        device = await context.pull(self.id, "Device")
        if not device:
            raise ValueError("No device connection handle supplied to PFJSigGen Config Chirp node.")
...
@register_node("visa/signal_generator/set_output")
class PFJSigGenSetOutputNode(BaseNode):
    ...
    async def execute(self, context: ExecutionContext, trigger_pin: str) -> Optional[str]:
        device = await context.pull(self.id, "Device")
        if not device:
            raise ValueError("No device connection handle supplied to PFJSigGen Set Output node.")
