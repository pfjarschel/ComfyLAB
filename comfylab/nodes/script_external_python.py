...
@register_node("script/python_external")
class ExternalPythonScriptNode(BaseSubprocessScriptNode):
    icon = "🔀"
    ...
    def _get_subprocess_args(self, script_file_path: str) -> List[str]:
        node_exec = self.properties.get("executable_path", "").strip()
        config = get_config()
        config_exec = config.get("external_python_path", "").strip()
        python_exe = node_exec or config_exec or sys.executable
