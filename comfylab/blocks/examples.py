...
"""
ComfyLAB Example Blocks - Educational Reference Implementation

This file contains example blocks that demonstrate key concepts for creating
custom ComfyLAB blocks. Each example is thoroughly commented to help newcomers
understand the patterns and best practices.

Examples included:
- RangeCheckBlock: Conditional execution outputs (multiple ExecOut pins)
- CSVWriterBlock: File I/O with proper teardown
- ArrayStatsBlock: Array processing with multiple data outputs
- CounterBlock: Stateful processing across executions
"""
...
@register_block("logic/range_check")
class RangeCheckBlock(BaseBlock):
    """
    ...
    async def pull_data(self, context: ExecutionContext, pin_name: str) -> Any:
        # Serve the boolean result when downstream blocks query it
...
@register_block("Numeric Arrays/operations/stats")
class ArrayStatsBlock(BaseBlock):
    """
    ...
    async def pull_data(self, context: ExecutionContext, pin_name: str) -> Any:
        # Serve computed statistics when downstream blocks query them
        if pin_name == "Min":
            return self._min
        elif pin_name == "Max":
            return self._max
...
@register_block("utility/counter")
class CounterBlock(BaseBlock):
    ...
    - How state persists between block executions within a run
    """
    icon = "🔢"
