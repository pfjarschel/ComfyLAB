...
ComfyLAB resolves this by providing a clean, modular graphical interface where users drag functional **Blocks** onto a digital canvas and connect them with visual **Wires**.
...
│ - Real-Time Interactive Plotting & Data Displays │
│ - Whiteboard Overlay & Annotations               │
└────────────────────────┬─────────────────────────┘
                         │ WebSocket / HTTP API
┌────────────────────────▼─────────────────────────┐
...
│ - Multi-Language Scripting & Native DLL / SO     │
└──────────────────────────────────────────────────┘
```

* **Front-End Canvas**: Runs in modern web browsers (or single-file Desktop app builds). Provides fluid graphical manipulation, auto-layout tools, interactive graph zoom/pan, and digital whiteboard annotations (user interaction layer).
* **Backend Server**: Powered by Python and FastAPI. Handles real-time hardware locks, VISA/SCPI queries, heavy matrix calculations, multi-threaded execution, file persistence, and Python virtual environment management (processing, instrument orchestration, and calculation layer).

---

### 1.3 Built-in Safety & Security

* **Instrument Protection & Safety Routines**: Physical laboratory hardware (lasers, high-voltage power supplies, RF signal sources) requires strict safety management. If a measurement run encounters an error, is manually stopped, or experiences a connection loss, ComfyLAB automatically executes safe shutdown hooks to turn off active outputs and restore instruments to safe states.
...
* **Secure Remote Access**: ComfyLAB supports secure remote operation over networks using token authentication, allowing researchers to safely monitor and control experiments running on lab computers from remote workstations. *(Note: HTTPS transport layer encryption is planned for future releases).*
...
| :--- | :--- |
| **Physical Hardware Control** | Full support for **VISA** (GPIB, USB TMC, Ethernet/LXI, RS-232/RS-485 Serial) and **SCPI** standard command sets. Built-in driver blocks for popular commercial oscilloscopes, multimeters, power supplies, and function generators. |
...
| **Multi-Language Custom Scripting** | Create inline **Script Blocks** in 9 programming languages: Python, Rust, JavaScript, TypeScript, Julia, R, Lua, Octave, and Wolfram. |
...
| **Rich Visualization & Data Export** | Real-time live line plotters (single/multi-trace, log/linear axes), 2D matrix image viewers, table displays, and automatic data exports to CSV, JSON, and Apache Parquet format. |
...
| **GUI & Plotting Setup** | Manual (PyQt, Tkinter, Matplotlib) | Integrated Front Panel | Integrated Figure Windows | **Instant Live Plotting & Image Viewer Widgets** |
...
| **Custom Code Extension** | Native | Complex C/Python Call Nodes | S-Functions / MATLAB Code | **Inline Script Blocks (Python, C/C++ DLL, Julia, etc.)** |
...
### Key Takeaways:
1. **Vs. Pure Python**: Python is powerful, but creating user interfaces, real-time interactive plots, hardware state loops, and visual workflows requires hundreds of lines of boilerplate code. ComfyLAB keeps Python's full analytical power while providing an immediate visual UI.
2. **Vs. LabVIEW**: LabVIEW is widely used but suffers from expensive commercial licenses, heavy proprietary binary files that break version control, and lock-in. ComfyLAB offers a modern, open-source alternative with JSON-based blueprints and seamless web browser integration.
...
```
┌──────────────────────────────────────────────────────────────┐
│ TOP TOOLBAR: [🟰][⚙️] | [▶ Run/⏸ Pause] [🛑 Stop] | [Theme]  │
├────────────────┬──────────────────────────────┬──────────────┤
│ SIDEBAR        │ MAIN CANVAS AREA             │ INSPECTOR    │
│ (Block Palette)│                              │ (Pin Values) │
│                │  ┌───────┐   Exec Wire       │              │
│ Search...      │  │ Entry │▶───┐              │ Inputs       │
│ Control Flow   │  └───────┘    │              │ Live Data    │
│ Math & Logic   │            ┌──▼────────────┐ │ Console Logs │
│ VISA Hardware  │ Data Wire  │ Math Block    │ │              │
│ Instruments    │ ──────────▶└───────────────┘ │              │
│ Visualization  │                              │              │
│                │ [Minimap]        [Tools 1-4] │              │
└────────────────┴──────────────────────────────┴──────────────┘
...
* **Top Toolbar**: Controls for running (▶️), pausing (⏸️), and stopping (🛑) blueprints, creating/saving workspace files (`Ctrl+S`), changing color themes, and configuring global VISA settings.
* **Sidebar (Left)**: Searchable palette containing all available built-in blocks, custom published blocks, and script nodes categorized by domain.
...
* **Block Inspector (Right)**: Contextual detail panel that opens when a block is selected. Displays live pin values, block parameters, brief documentation, and execution console logs.
...
| :--- | :--- |
| 🔄 **Control Flow** | For Loop, While Loop, If/Else Branch, Delay/Sleep, Stop Execution. |
...
| 📊 **Arrays & Signal** | Array Create, Range/Linspace, Array Slicing, FFT Power Spectrum, Lowpass/Bandpass Filters, Detrend, Statistics (Mean, Std, Min, Max), Peak Detector, Curve Fitting (Gaussian, Exponential, Polynomial, Custom). |
| 💾 **File I/O** | Read/Write CSV files, JSON I/O, Parquet storage, Text File Read/Write, Image Loader/Saver. |
| 📡 **VISA & Hardware** | VISA Resource Open, SCPI Write, SCPI Read, SCPI Query, VISA Close, Serial Port Open/Read/Write. |
| 🔬 **Instrument Drivers** | Oscilloscope Setup & Waveform Read, Digital Multimeter (DMM) Read, DC Power Supply Set/Read, Function Generator Setup, and many more. |
| 📈 **Display & UI** | Interactive Line Plotter (Single & Multi-trace, Log/Linear axes), Image Matrix Viewer, Data Table Viewer, Digital Gauge, Status Indicator. |
| ⚙️ **Native & Scripting** | Native Library (DLL/SO) Invocation, Polyglot Script Node (Python, JS, Julia, Rust, etc.). |
...
8. Click **Run Blueprint** (▶️) on the top toolbar.
...
2. Set the `For Loop` iterations count to `50`.
...
4. Enter the Python code snippet shown above.
