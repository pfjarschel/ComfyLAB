...
export const ActionBlock = ({ id, data, selected }: NodeProps<any>) => {
  ...
  const { setNodes: setBlocks, getNode: getBlock } = useReactFlow();
  ...
  // Dynamic Inputs: augment layout for blocks using "dynamic_inputs" ui_behavior
  if (layout && layout.ui_behavior?.dynamic_inputs) {
    ...
    // Default to the block's configured itemCount property, or the behavior's default_count
  ...
  const isMissing = !!(blockRegistry && !registryLayout);
  ...
  useEffect(() => {
    updateBlockInternals(id);
  }, [id, pinsHash, connectedEdgesHash, updateBlockInternals]);
  ...
  const getConnectedPinValue = (pinName: string) => {
    ...
    const sourceBlock = getBlock(edge.source);
  ...
  // Sync calculated minimum height back to the React Flow block's parent container style minHeight
  useEffect(() => {
    setBlocks((nds) =>
      nds.map((n) => {
        if (n.id === id) {
  ...
  }, [id, calculatedMinHeight, setBlocks]);

  // Re-measure block layout internals whenever minimum height constraint changes
  useEffect(() => {
    updateBlockInternals(id);
  }, [id, calculatedMinHeight, updateBlockInternals]);
  ...
  useEffect(() => {
    const handleTelemetry = (e: any) => {
      ...
      if (status) {
        const containerEl = document.getElementById(`block-container-${id}`);
        if (containerEl) {
          containerEl.classList.remove('idle', 'running', 'success', 'error', 'stopped');
          containerEl.classList.add(status);
        }
        const statusEl = document.getElementById(`block-status-${id}`);
        if (statusEl) {
      ...
      if (ledEl) {
        ...
        const isStateTrue = !!(block?.data?.results as any)?.state;
      ...
      // We DO NOT trigger a React re-render of this block here.
      // This is a CRITICAL memory optimization. Re-rendering a 1500-line block component
      ...
      // and pull data from blocksRef (via getNode).
    };
  ...
  if (isMissing) {
    ...
    return (
      <div
        className={`glass-panel action-block missing-block ${selected ? 'selected' : ''}`}
        style={{
          minHeight: `${calculatedMinHeight}px`,
        }}
      >
        <span 
          className={`block-pin-icon ${data.pinned ? 'pinned' : ''}`}
          onClick={(e) => { 
            e.stopPropagation(); 
            handleChange('pinned', !data.pinned); 
          }}
          title={data.pinned ? "Unlock block" : "Pin block in place"}
        >
          📌
        </span>
        ...
        {/* --- BODY --- */}
        <div className="action-block-body" style={{ color: '#fca5a5', fontSize: '0.8rem', gap: '6px' }}>
          ...
          <div style={{ fontSize: '0.75rem', opacity: 0.8, fontStyle: 'italic', marginTop: '4px', lineHeight: 1.3 }}>
            Warning: This block type is not found in the registry.
          </div>
  ...
  if (isLED) {
    ...
    return (
      <div
        id={`led-${id}`}
        className={`led-indicator-block ${selected ? 'selected' : ''}`}
        style={{
          width: '40px',
          height: '40px',
          borderRadius: '50%',
          background: ledColor,
          border: `3px solid ${selected ? '#3b82f6' : 'var(--block-border)'}`,
  ...
  if (isAnyPassthrough) {
    ...
    return (
      <div
        className={`passthrough-block ${isExec ? 'exec' : 'data'} ${selected ? 'selected' : ''}`}
        style={{
          width: '14px',
          height: '14px',
          borderRadius: '50%',
  ...
  return (
    <div
      ...
      className={`glass-panel action-block ${data.status || 'idle'} ${isCluster ? 'cluster-block' : ''} ${data.pinned ? 'pinned-block' : ''} ${data.isPersistent ? 'persistent-block' : ''} ${data.disabled ? 'disabled-block' : ''} ${selected ? 'selected' : ''}`}
      onDoubleClick={(e) => {
        // Do not open inspector when double-clicking interactive elements
        const target = e.target as HTMLElement;
        if (target.closest('input, select, textarea, button, .nodrag, .toggle-switch, .optional-toggle, .block-pin-icon, a')) {
          return;
        }
        if (data.onInspect) {
          data.onInspect(id);
    ...
    >
      ...
      >
        <div className="block-icon">{layout.icon}</div>
        {isEditingName ? (
          <input
            type="text"
            value={tempName}
          ...
          <>
            ...
            {data.disabled && (
              <span
                className="block-disabled-badge"
                title="Block is disabled (execution bypassed, outputs default to empty)"
                style={{
                  fontSize: '0.75rem',
                  marginLeft: '4px',
                  opacity: 0.9,
            ...
            {data.isPersistent && (
              <span 
                className="block-persistence-lock" 
                title="Persistent block (kept in memory between runs)"
                style={{
                  fontSize: '0.75rem',
                  marginLeft: '4px',
                  opacity: 0.9,
            ...
            <span 
              className="block-name-edit-icon" 
              onClick={(e) => { 
                e.stopPropagation(); 
                setIsEditingName(true); 
                setTempName(data.customName || layout.name); 
